...
from const import DIALOGUE_TONE_MAP,PLAYER_NAMES,ROLE_FACTION,PLAYER_POSITIONS,FRONTEND_PHASE_MOON,PLAYER_HOUSES,PLAYER_AVATARS
...
class LiveGameEngine(GameEngine):
    ...
    def _on_game_log(self, phase: str, agent_id: Optional[int], log_type: str, content: Any):
        if log_type == "speech" and isinstance(content, str) and agent_id is not None:
            ...
            if action_type == "vote":
                ...
                explain = str(action.get("explain") or action.get("reason") or "").strip()
                hidden_reasons = {"invalid_or_missing_target", "tool_not_available:abstain", "mcp_empty_action"}
                if explain and explain not in hidden_reasons:
                    self._dialogue_seq += 1
                    speaker_name = PLAYER_NAMES[agent_id] if agent_id < len(PLAYER_NAMES) else f"玩家{agent_id}"
                    tone = "night" if phase == "night" else "vote"
                    phase_text = "夜间行动" if phase == "night" else "本轮投票"
                    dialogue_entry = {
                        "id": f"d{self._dialogue_seq}",
                        "speakerId": agent_id,
                        "tone": tone,
                        "text": f"{speaker_name}选择保留{phase_text}：{explain}",
                        "timestamp": f"Day {self.game_state.get('day', 1)} {datetime.now().strftime('%H:%M')}",
                    }
                    self._accumulated_dialogues.append(dialogue_entry)
                    self._queue.put({"type": "dialogue", "data": dialogue_entry})
    ...
    def _notify_frontend_phase(self, phase_label: str):
        self._broadcast_snapshot(phase_label)
